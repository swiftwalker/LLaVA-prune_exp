...
TARGET_DIR="$METHOD_DIR/third_party/divprune"
COMMIT_FILE="$METHOD_DIR/third_party/divprune_FETCHED_COMMIT.txt"
DRY_RUN=0
...
echo "Official DivPrune repo: $REPO_URL"
...
current="$(git -C "$TARGET_DIR" rev-parse HEAD)"
echo "$current" > "$COMMIT_FILE"
echo "Fetched official DivPrune at $current"
