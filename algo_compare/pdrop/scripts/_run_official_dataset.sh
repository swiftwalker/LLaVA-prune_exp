...
args=(--method pdrop --dataset "$DATASET" --python-bin "$RUNNER_PYTHON")
...
[[ -n "${PDROP_LAYER_LIST:-}" ]] && args+=(--pdrop-layer-list "$PDROP_LAYER_LIST")
[[ -n "${PDROP_IMAGE_TOKEN_RATIO_LIST:-}" ]] && args+=(--pdrop-image-token-ratio-list "$PDROP_IMAGE_TOKEN_RATIO_LIST")
