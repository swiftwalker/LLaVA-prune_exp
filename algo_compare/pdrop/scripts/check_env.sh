...
OFFICIAL_REPO="${OFFICIAL_REPO:-$METHOD_DIR/third_party/PyramidDrop}"
...
if [[ -d "$OFFICIAL_REPO/.git" ]]; then
  echo "OK      official repo git checkout: $OFFICIAL_REPO"
elif [[ -d "$OFFICIAL_REPO" && -f "$COMMIT_FILE" ]]; then
  echo "OK      official repo archive checkout: $OFFICIAL_REPO ($(<"$COMMIT_FILE"))"
else
  echo "MISSING official repo: $OFFICIAL_REPO"
  status=1
fi
...
if [[ "$status" -eq 0 && -x "$PYTHON_BIN" ]]; then
  PYTHONPATH="$OFFICIAL_REPO:${PYTHONPATH:-}" "$PYTHON_BIN" - <<'PY' || status=1
import importlib
for name in ["torch", "transformers", "shortuuid", "llava"]:
...
if [[ "$status" -eq 0 ]]; then
  ...
  echo "Environment check found missing paths or imports. This command is read-only."
fi
