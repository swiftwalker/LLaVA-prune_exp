...
QUESTION_FILE="${QUESTION_FILE:-$REPO_ROOT/entropy_exp/eval_questions/gqa/llava_gqa_testdev_balanced.jsonl}"
IMAGE_FOLDER="${IMAGE_FOLDER:-$REPO_ROOT/entropy_exp/datasets/gqa/images}"
USE_VERSION="${USE_VERSION:-2_0}"
RETAIN_TOKN="${RETAIN_TOKN:-192}"
CONV_MODE="${CONV_MODE:-vicuna_v1}"
TEMPERATURE="${TEMPERATURE:-0}"
RUN_LABEL="${RUN_LABEL:-gqa_${MODEL_NAME}_v${USE_VERSION}_retain${RETAIN_TOKN}_$(date +%Y%m%d_%H%M%S)}"
...
python3 "$METHOD_DIR/scripts/collect_results.py" --run-dir "$OUTPUT_DIR" --dataset gqa
