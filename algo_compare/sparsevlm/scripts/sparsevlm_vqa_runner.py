#!/usr/bin/env python3
"""Run official SparseVLM-v1/v2 on local VQA-style datasets with benchmark stats."""
...
def _add_runtime_paths() -> None:
    official_repo = Path(os.environ.get("SPARSEVLM_OFFICIAL_REPO") or os.getcwd()).resolve()
...
def eval_model(args: argparse.Namespace) -> None:
    ...
    benchmark = BenchmarkRecorder.from_args(args, answers_file=answers_file, method="sparsevlm", dataset=args.dataset)
    ...
    with answers_file.open("w", encoding="utf-8") as ans_file:
        for sample_idx, raw_item in enumerate(tqdm(questions)):
            ...
            benchmark.finish_sample(
                ...
                metadata={
                    "retain_token": args.retained_tokens,
                    "use_version": os.environ.get("USE_VERSION"),
                    "sparsevlm_enabled": image_tensor is not None,
                },
            )
