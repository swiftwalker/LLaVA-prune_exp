...
TARGET_DIR="$METHOD_DIR/third_party/CDPruner"
COMMIT_FILE="$METHOD_DIR/third_party/CDPruner_FETCHED_COMMIT.txt"
DRY_RUN=0
...
echo "Official CDPruner repo: $REPO_URL"
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  if [[ -d "$TARGET_DIR/.git" ]]; then
    ...
    echo "Would clone and checkout the pinned commit."
  fi
  exit 0
fi
...
current="$(git -C "$TARGET_DIR" rev-parse HEAD)"
printf '%s\n' "$current" > "$COMMIT_FILE"
echo "Fetched official CDPruner at $current"
