...
args=(--method fastv --dataset "$DATASET" --python-bin "$RUNNER_PYTHON")
...
[[ -n "${FASTV_K:-}" ]] && args+=(--fastv-k "$FASTV_K")
[[ -n "${FASTV_R:-}" ]] && args+=(--fastv-r "$FASTV_R")
[[ -n "${FASTV_ATTENTION_RANK:-}" ]] && args+=(--fastv-attention-rank "$FASTV_ATTENTION_RANK")
[[ -n "${FASTV_IMAGE_TOKEN_LENGTH:-}" ]] && args+=(--fastv-image-token-length "$FASTV_IMAGE_TOKEN_LENGTH")
[[ -n "${FASTV_SYS_LENGTH:-}" ]] && args+=(--fastv-sys-length "$FASTV_SYS_LENGTH")
[[ -n "${FASTV_MODE:-}" ]] && args+=(--fastv-mode "$FASTV_MODE")
[[ -n "${FASTV_MAX_EXPANDED_TOKENS:-}" ]] && args+=(--fastv-max-expanded-tokens "$FASTV_MAX_EXPANDED_TOKENS")
