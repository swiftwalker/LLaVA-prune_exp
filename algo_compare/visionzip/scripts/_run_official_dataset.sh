...
args=(--method visionzip --dataset "$DATASET" --python-bin "$RUNNER_PYTHON")
...
[[ -n "${VISIONZIP_DOMINANT:-}" ]] && args+=(--visionzip-dominant "$VISIONZIP_DOMINANT")
[[ -n "${VISIONZIP_CONTEXTUAL:-}" ]] && args+=(--visionzip-contextual "$VISIONZIP_CONTEXTUAL")
