...
TARGET_DIR="$METHOD_DIR/third_party/VisionZip"
COMMIT_FILE="$METHOD_DIR/third_party/VisionZip_FETCHED_COMMIT.txt"
DRY_RUN=0
...
echo "Official VisionZip repo: $REPO_URL"
...
current="$(git -C "$TARGET_DIR" rev-parse HEAD)"
echo "$current" > "$COMMIT_FILE"
echo "Fetched official VisionZip at $current"
